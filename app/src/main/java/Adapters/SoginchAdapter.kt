...
import kotlinx.android.synthetic.main.item_soginch.view.*

class SoginchAdapter(var context: Context, var list: ArrayList<SoginchModel>) :
    RecyclerView.Adapter<SoginchAdapter.MyViewHolder>() {
    inner class MyViewHolder(var itemview: View) : RecyclerView.ViewHolder(itemview) {
        fun onBind(model: SoginchModel, position: Int) {
            itemview.soginch_sher_name.text = list[position].name
            itemview.soginch_sher_qator.text = list[position].code


            itemview.setOnClickListener {
            //    itemview.findNavController().navigate(R.id.soginchInfoFragment, bundleOf("keyName" to list[position].name, "keyCode" to list[position].code ))

                dialogShow(SoginchModel("${itemview.soginch_sher_name.text.toString()}","${itemview.soginch_sher_qator.text.toString()}"))

            }
        }
    }

    fun dialogShow(sher: SoginchModel){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(
            LayoutInflater.from(parent.context).inflate(R.layout.item_soginch, parent, false)
        )
    }
