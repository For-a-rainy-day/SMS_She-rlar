...
import kotlinx.android.synthetic.main.item_shield_sevgi.*
import kotlinx.android.synthetic.main.item_shield_sevgi.view.*

class SevgiAdapter(var context: Context, var list: ArrayList<SevgiModel>) :
        RecyclerView.Adapter<SevgiAdapter.MyViewHolder>() {
    inner class MyViewHolder(var itemview: View) : RecyclerView.ViewHolder(itemview) {
        fun onBind(model: SevgiModel, position: Int) {
            ...
            itemview.setOnClickListener {
             //   itemview.findNavController().navigate(R.id.sevgiInfoFragment, bundleOf("keyName" to list[position].name, "keyCode" to list[position].code ))
                ...
                dialogShow(SevgiModel("${itemview.tariffName.text.toString()}","${itemview.tariffCode.text.toString()}"))
